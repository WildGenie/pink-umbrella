                ...
                if (peers.Count > 0)
                {
                    foreach (var peer in peers)
                    {
                        if (peer != null)
                        {
                            ...
                            try
                            {
                                var profiles = await client.GetProfiles(LastTimeSyncedProfiles, await authService.GetKeyPair(peer.PublicKey));
                ...
                if (peers.Count > 0)
                {
                    foreach (var peer in peers)
                    {
                        if (peer != null)
                        {
                            ...
                            try
                            {
                                var items = await client.GetPosts(LastTimeSyncedPosts, await authService.GetKeyPair(peer.PublicKey));
                ...
                if (peers.Count > 0)
                {
                    foreach (var peer in peers)
                    {
                        if (peer != null)
                        {
                            ...
                            try
                            {
                                var medias = await client.GetArchivedMedia(LastTimeSyncedArchivedMedia, await authService.GetKeyPair(peer.PublicKey));
                ...
                if (peers.Count > 0)
                {
                    foreach (var peer in peers)
                    {
                        if (peer != null)
                        {
                            ...
                            try
                            {
                                var items = await client.GetShops(LastTimeSyncedShops, await authService.GetKeyPair(peer.PublicKey));
                ...
                if (peers.Count > 0)
                {
                    foreach (var peer in peers)
                    {
                        if (peer != null)
                        {
                            ...
                            try
                            {
                                var items = await client.GetPeers(LastTimeSyncedPeers, await authService.GetKeyPair(peer.PublicKey));
